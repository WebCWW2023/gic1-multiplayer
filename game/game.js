...
var screenButtonLeft = document.querySelector(".screenButtonLeft");
...
const htmlEvents = () => {
  ...
  screenButtonToggle.addEventListener("click", () => {
    if (screenButtonLeft.style.display !== "flex") {
      screenButtonLeft.style.display = "flex";
      screenButtonToggle.innerHTML = '<i class="fas fa-times"></i>';
    } else {
      screenButtonLeft.style.display = "none";
...
const init = () => {
  ...
  directionalLightM(gui, lightGroup, helperGroup, -80, 150, 0, 0, 0, 0, 0.6, 0xffffff, lightGui);
  ...
  const glbLoader = (name) => {
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        water.position.set(336.9, 3.4, 50.2);
        water.scale.set(9.8, 20.1, 1);
        water.name = 'water';
        cityGroup.add(water);
        /*------------------------------------*/
